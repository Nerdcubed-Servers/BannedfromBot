...
maxposts = 100
wait = 20
reply = "He's banned from many places especially: "

with open("r", "countries.csv") as countries:
	list_of_countries = countries.readlines()
    ...
    for post in posts:
        ...
        try:
            ...
            if not cur.fetchone():
                ...
                if match and not notmatch and not subredditbans and postauthor.lower() != username.lower() and not postauthor.lower() in alternativebot.lower:
                    print('Found one! Replying to ' + pid + ' by ' + postauthor)
                    random_no = randint(0, len(list_of_countries))
                    post.reply(reply, list_of_countries[random_no])
